...
class OrderRepository {
  Future<List<Order>> getOrders() async {
    ...
    if (response.data is! List) throw FormatException('Expected List from /orders, got ${response.data.runtimeType}');
  ...
  Future<Order> placeOrder(Map<String, dynamic> orderData) async {
    ...
    if (response.data is! Map) throw FormatException('Expected Map from /orders, got ${response.data.runtimeType}');
    return Order.fromJson(response.data);
  }

  Future<List<Order>> getMyOrders() async {
    final response = await apiService.dio.get('/orders/my');
    if (response.data is! List) throw FormatException('Expected List from /orders/my, got ${response.data.runtimeType}');
    return (response.data as List).map((i) => Order.fromJson(i)).toList();
  }

  Future<Order> updateOrderStatus(String id, String status) async {
    final response = await apiService.dio.patch('/orders/$id/status', data: {'status': status});
    if (response.data is! Map) throw FormatException('Expected Map from /orders/$id/status, got ${response.data.runtimeType}');
    return Order.fromJson(response.data);
  }
  ...
  Future<Order> updateOrder(String id, Map<String, dynamic> orderData) async {
    ...
    if (response.data is! Map) throw FormatException('Expected Map from /orders/$id, got ${response.data.runtimeType}');
    return Order.fromJson(response.data);
  }
  ...
  Future<List<Order>> getAvailableJobs() async {
    ...
    if (response.data is! List) throw FormatException('Expected List from /orders (jobs), got ${response.data.runtimeType}');
    return (response.data as List).map((i) => Order.fromJson(i)).toList();
  }
  ...
  Future<List<Order>> getRiderOrders(String riderId) async {
    ...
    if (response.data is! List) throw FormatException('Expected List from /orders (rider), got ${response.data.runtimeType}');
    return (response.data as List).map((i) => Order.fromJson(i)).toList();
  }
}
